...
function generateToken(host, login, password){
	applicationName = "Nuxeo%20Adobe%20Connector";
	...
	$.ajax({
		 ...
		 success: function (data) {
			 if(data == null || data == ""){
	        	 ...
	        	 var extScript = "$._ext_PHXS.setToken('"+host+"', '"+login+"', '"+data+"')";
...
function initializeToken(){		
	var extScript = "$._ext_PHXS.getToken()";
    new CSInterface().evalScript(extScript, function(result){
        ...
        if(result != ""){
            ...
            if(token == null || token == ""){
    			...
    			$("#host-message").html("on "+host+" as "+login);
    			$("#host-message").show();
    			//No link persistency with Photoshop
...
function revokeToken(){
	 var extScript = "$._ext_PHXS.setToken('', '', '')";
	 new CSInterface().evalScript(extScript);
	 host = "";
     token = "";
     login = "";
